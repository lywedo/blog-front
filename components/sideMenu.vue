<template>
  <div class="menu">
    <h1>
      ...
      <div class="fa fa-arrow-left" aria-hidden="true" @click="$router.go(-1)">
        返回
      </div>
    </h1>
    <ul class="files">
      <template v-for="i in articleList.length">
        <li :class="{ current: articleList[i - 1].current }" :key="i.id">
          <span @click="selectThis(i)">{{
    ...
    <div class="copy-right">Copyright © 2016 Jrain Lau</div>
  </div>
</template>
